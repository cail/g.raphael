...
Raphael.fn.g.linechart = function (x, y, width, height, valuesx, valuesy, opts) {
    ...
    var allx = Array.prototype.concat.apply([], valuesx),
        ...
        maxy = opts.maxy !== undefined ? opts.maxy : ydim.to,
    ...
    if (opts.axis) {
        ...
        +ax[1] && axis.push(this.g.axis(x + width - gutter, y + height - gutter, height - 2 * gutter, miny, maxy, opts.axisystep || Math.floor((height - 2 * gutter) / 20), 3, opts.labels && opts.labels[1]));
        +ax[2] && axis.push(this.g.axis(x + gutter, y + height - gutter, width - 2 * gutter, minx, maxx, opts.axisxstep || Math.floor((width - 2 * gutter) / 20), 0, opts.labels && opts.labels[2]));
        +ax[3] && axis.push(this.g.axis(x + gutter, y + height - gutter, height - 2 * gutter, miny, maxy, opts.axisystep || Math.floor((height - 2 * gutter) / 20), 1, opts.labels && opts.labels[3]));
    }
    var lines = this.set(),
        symbols = this.set(),
        line;
    ...
    function createColumns(f) {
        ...
        Xs.sort(function(a, b){ return a - b; }); // http://github.com/DmitryBaranovskiy/g.raphael/issues#issue/9
        // remove duplicates
        var Xs2 = [],
            xs = [];
